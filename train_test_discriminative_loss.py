...
from training_utils import train, accuracy_evaluation
from loss_function import DiscriminativeLoss
...
loss_dis = DiscriminativeLoss(delta_dist=6.0)
if device=="cuda":
    loss_dis.cuda()
...
for t in range(epochs):
    print(f"Epoch {t+1}\n-------------------------------")
    _train_batch_loss , _train_batch_accuracy = train(train_loader, model, loss_fn, optimizer, loss_dis, 1., device)
    scheduler.step()
